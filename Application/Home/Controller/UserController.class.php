...
// 				$error = '未知错误';
	...
	public function admin_user() {
		...
		$userdata = M ( 'user' )->where ( $map )->order ( 'uid desc' )->page ( $page, $row )->select ();
		...
		$currentName = get_nickname ( $this->mid );
		$this->assign ( 'nickname', $currentName );
		$this->assign ( 'is_audit', $audit );
		$this->display ();
	}
	function changeStatus() {
		...
		$res = D ( 'Common/User' )->updateInfo ( $id, $savedata );
		if ($res) {
			$this->success ( '修改成功！' );
		} else {
			$this->error ( '修改失败！' );
	...
	function del_user() {
		...
		$res = D ( 'Common/User' )->where ( array (
				'uid' => $uid 
		) )->delete ();
		if ($res) {
			D ( 'Common/User' )->clear ( $uid );
			$this->success ( '删除成功！' );
		} else {
			$this->error ( '删除失败！' );
		}
