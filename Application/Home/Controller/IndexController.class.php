...
class IndexController extends HomeController {
	...
	public function main() {
		...
		// 商城版的直接跳转到商城首页
		$url = addons_url ( 'Shop://Shop/summary' );
		redirect ( $url );
...
//		$count ['total'] = M ()->where ( $map )->table ( $px . 'public_follow as f' )->join ( $px . 'user as u ON f.uid=u.uid' )->count ();
	...
	function test_token() {
		...
		foreach ( $list as $vo ) {
			$url = 'https://api.weixin.qq.com/cgi-bin/getcallbackip?access_token=' . get_access_token_by_apppid ( $vo ['appid'], $vo ['secret'] );
			$res = wp_file_get_contents ( $url );
			// addWeixinLog ( $res, 'test_token' );
			$res = json_decode ( $res, true );
			if ($res ['errcode'] == '40001') {
				$res = get_access_token_by_apppid ( $vo ['appid'], $vo ['secret'], true );
				// addWeixinLog ( $res, 'test_token2' );
			}
		}
	}
