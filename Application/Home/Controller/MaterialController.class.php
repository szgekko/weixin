...
class MaterialController extends HomeController {
	...
	function _initialize() {
        ...
        if (empty($mdm)) {
            ...
            $mdmMap['url'] = 'Home/Material/material_lists';
            $mdmMap['url_type'] = 1;
            $menu = M('manager_menu')->where($mdmMap)->find();
            if (empty($menu['pid'])) {
                $mdm = $menu['id'];
            } else {
                $mdm = $menu['pid'] . '|' . $menu['id'];
            }
        }
        $this->mdm = $mdm;
		$param = array (
	...
	function text_add() {
		...
		if (IS_POST) {
			...
			$Model = $this->checkAttr ( $Model, $model ['id'] );
			if ($Model->create () && $id = $Model->add ()) {
				$this->success ( '添加' . $model ['title'] . '成功！', U ( 'text_lists?model=' . $model ['name'] . '&mdm='.$this->mdm ) );
			} else {
				$this->error ( $Model->getError () );
			}
		} else {
			...
			$this->assign ( 'post_url', U ( 'text_add',array('mdm'=>$this->mdm) ) );
	...
	function text_edit() {
		...
		if (IS_POST) {
			...
			$Model->create () && $res = $Model->save ();
			if ($res !== false) {
				$this->success ( '保存' . $model ['title'] . '成功！', U ( 'text_lists?model=' . $model ['name'] . '&mdm='.$this->mdm ) );
			} else {
				$this->error ( $Model->getError () );
			}
		} else {
			...
			$this->assign ( 'post_url', U ( 'text_edit',array('mdm'=>$this->mdm) ) );
