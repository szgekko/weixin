		...
		if (!isset($addons[$key])){
		   ...
		   if(!empty($customMenu)){
		       if ($customMenu['addon']){
		           ...
		           if ($customMenu['addon']=='Vote'){
		               $keywordArr['table_name']='shop_vote';
		           }
	...
	private function init_follow($data) {
		...
		if (! $uid && C ( 'USER_BASE_INFO' )) { // 如果有权限获取用户信息，即自动获取
			...
			if (isset ( $userInfo ['nickname'] )) {
				...
				$map ['has_subscribe']=$userInfo['subscribe'];
