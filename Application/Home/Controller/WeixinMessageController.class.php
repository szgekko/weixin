...
class WeixinMessageController extends HomeController {
	...
	public function lists() {
		...
		$token = get_token ();
		/* 查询记录总数 */
		$count = M ()->query ( "SELECT count(DISTINCT FromUserName) as num FROM `{$px}weixin_message` where `ToUserName`='{$token}' and `type`=0" );
...
// 		$list ['list_data'] = M ()->query ( $sql );
		...
		$map['type']=0;
		$maxData = M('weixin_message')->where($map)->group('FromUserName')->getFields('max(id)');
		if (!empty($maxData)){
		    unset($map);
		    $map['id']=array('in',$maxData);
		    $list ['list_data'] = M('weixin_message')->where($map)->order('is_read asc,id desc')->limit($limit)->select();
		}
		$dao = D ( 'Common/User' );
		foreach ( $list ['list_data'] as &$v ) {
			$user = $dao->getUserInfoByOpenid ( $v ['FromUserName'] );
