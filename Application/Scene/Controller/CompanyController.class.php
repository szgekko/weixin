...
class CompanyController extends Controller {
    ...
    public function companytpllist(){
		$this->unlogin();
		$_scene = M('scene');		
		$where1['userid_int']  = intval(session("mid"));
		$companyid = M('user')->where($where1)->getField('companyid');
	...
	public function sublist(){
		...
		$subuser = M('user')->where('companyid='.$company.'&&type=21')->order('userid_int desc')->select();
		//print_r($subuser);die();
	...
	public function subsave(){
		...
		M('user')->data($datainfo)->where($where)->save();
		//deptId id name
		echo '{"success":true,"code":200,"msg":"操作成功","obj":null,"map":null,"list":null}';
	}
