...
class UpfileController extends Controller {
	...
	public function upload(){
		...
		if(I('get.fileType',0)==2)
		{
			...
			$upload->savePath = 'mp3/'.session("mid").'/'; // 设置附件上传（子）目录
		}
		else if(I('get.fileType',0)==4)
		{
			$upload->exts = array('mp3');// 设置附件上传类型
			$upload->savePath = 'mp3/'.session("mid").'/'; // 设置附件上传（子）目录
		}
		else
		{
			...
			$upload->savePath = 'pic/'.session("mid").'/'; // 设置附件上传（子）目录
		}
		...
		if(!$info) {// 上传错误提示错误信息
			...
			foreach($info as $file){
				...
				$jsonstr = '{"success":true,"code":200,"msg":"success","obj":{"id":9386090,"name":"'.$file['savename'].'","extName":"'.strtoupper($file['ext']).'","fileType":0,"bizType":0,"path":"'.$file['savepath'].$file['savename'].'","tmbPath":"'.$thubimagenew.'","createTime":1426209412922,"createUser":"'.session("mid").'","sort":0,"size":'.$sizeint.',"status":1},"map":null,"list":null}';
