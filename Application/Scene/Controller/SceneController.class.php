...
class SceneController extends Controller{ 
    ...
    public function _initialize(){
		...
		if(intval(session('mid')) != 100)
		{
			//$wheresessionuser["userid_int"] = intval(session('mid'));
    ...
    public function view(){
		...
		{
			if($_scene_list2[0]['userid_int']!=intval(session('mid')))
			{
		...
		$advuserinfo['uid'] = $_scene_list2[0]['userid_int'];
		$advUser = M('user');
    ...
    "list": [';
		...
		foreach($_scene_list as $vo)
        {
			...
			if(strpos($vo["content_text"],'eqs\/link?id')!==false){
				$vo["content_text"]=str_replace('eqs\/link?id','index.php?s=/Scene/Scene/link&id',$vo["content_text"]);
    ...
    public function design(){
        ...
        if($sceneid&&$sceneid<1104){
            ...
            if(intval(session('mid'))!=1)
            {
                //$where2['userid_int']  = intval(session('mid'));
            }
            $where2['delete_int']  = 0;
			...
			'"scene": {"id": '.$_scene_list2[0]['sceneid_bigint'].',"name": '.json_encode($_scene_list2[0]['scenename_varchar']).',"createUser": "'.$_scene_list2[0]['userid_int'].'","createTime": 1425998747000,"type": '.$_scene_list2[0]['scenetype_int'].',"pageMode": '.$_scene_list2[0]['movietype_int'].',"image": {"imgSrc": "'.$_scene_list2[0]['movietype_int'].'","isAdvancedUser": false';
			if($_scene_list2[0]['musicurl_varchar']!=''){
				$jsonstr = $jsonstr.',"bgAudio": {"url": "'.$_scene_list2[0]["musicurl_varchar"].'","type": "'.$_scene_list2[0]["musictype_int"].'"}';
			}
			$bgAudiotcc='';
			...
			$jsonstr = $jsonstr.'},"isTpl": '.$isTpl.',"isPromotion": 0,"status": 1,"openLimit": 0,	"submitLimit": 0,	"startDate": null,	"endDate": null,	"accessCode": null,	"thirdCode": null,	"updateTime": 1426038857000,	"publishTime": 1426038857000,	"applyTemplate": 0,	"applyPromotion": 0,	"sourceId": null,	"code": "'.$_scene_list2[0]['scenecode_varchar'].'",	"description": "'.
				str_replace("\r\n","\\r\\n",$_scene_list[0]['desc_varchar']).'",	"sort": 0,"pageCount": 0,	"dataCount": 0,	"showCount": '.$_scene_list2[0]['hitcount_int'].', '.$bgAudiotcc.'	"userLoginName": null,"userName": null}},	"map": null,"list": null}';
			echo $jsonstr;
		}
    }
    ...
    public function detail(){
		...
		$jsonstr_property='';
		if(C('JS_VISION')>=3.4&& $_scene_list[0]['property']){
		...
		$jsonstr = '{
			...
			"obj": {
				...
				"isTpl": 0,
				"isPromotion": 0,
				"isPublic": '.$_scene_list[0]['is_public'].',
	...
	public function createpage(){
		...
		if((session('level_int')=='4') ){	
		   ...
		   $getid = I('get.id',0);
			...
			$datainfo['userid_int'] = session('mid');
			$result = $_scenepage->add($datainfo);
			if($result){
				$where_plist="sceneid_bigint='".$_scene_list[0]['sceneid_bigint']."' AND userid_int='".session('mid')."' AND pageid_bigint<>'$result' AND pagecurrentnum_int>".$pagenumY." " ;
    ...
    public function delpage(){
		...
		if((session('level_int')==4&& session('type')==1) && !I('get.ismy')){	
			...
			if(intval(session('mid'))!=1)
			{
				$map['userid_int']  = intval(session('mid'));
			}
