        ...
        <!-- 高级搜索 -->
         ...
         <form  class="sleft mr10" action="" method="post">
			              <select name="goods_category" style="padding:3px 4px; margin:0; width:150px; border:none" >
			              	<option value="0" <?php if($_GET['goods_category']==0)echo 'selected=selected'; ?>>按分类查看</option>
			              	<?php if(!empty($goods_category)): if(is_array($goods_category)): $i = 0; $__LIST__ = $goods_category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($_GET['goods_category']==$vo['id']){echo 'selected=selected';} ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
			              </select>
			</form>

            <?php if($search_button): ?><div class="sleft">
                ...
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo ($search_url); ?>"><i class="btn-search"></i></a> </div><?php endif; ?>
        </div>
        <!-- 多维过滤 -->
...
$(function(){
	...
	$("select[name='goods_category']").change(function(){
		var url = '<?php echo addons_url("Shop://Goods/lists", array("type"=>$_GET["type"]));?>&mdm=<?php echo ($_GET["mdm"]); ?>&goods_category='+$(this).val();
