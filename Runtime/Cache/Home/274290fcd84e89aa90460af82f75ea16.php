...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?> 
      <?php if($add_button || $del_button || $search_button || !empty($top_more_button)): ?><div class="table-bar">
        <div class="fl">
          ...
          <a class="btn" href="<?php echo U(lists,array('deposit_pay'=>1,'mdm'=>$_GET['mdm']));?>">全额支付</a>
          <a class="btn" href="<?php echo U(lists,array('deposit_pay'=>2,'mdm'=>$_GET['mdm']));?>">订金支付</a>
          <a class="btn" href="<?php echo U(lists,array('deposit_pay'=>3,'mdm'=>$_GET['mdm']));?>">待支付</a>
        ...
        <?php if($search_button): ?><div class="search-form fr cf">
          <form action="" method="post" class="sleft mr10">
            <select name="users" style="padding:3px 4px; margin-bottom:5px; width:150px" >
              <option value="0" <?php if($_GET['users']==0)echo 'selected=selected'; ?>>全部用户</option>
                  <?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["manager_id"]); ?>" <?php if($_GET['users']==$vo['manager_id']){echo 'selected=selected';} ?> ><?php echo ($vo["nickname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
			   </form>    
          <div class="sleft">
      ...
      <ul class="article_card_list">
            ...
            <thead>
              <tr>
                ...
                <?php if(is_array($list_grids)): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><th <?php if(!empty($field["width"])): ?>width="<?php echo ($field["width"]); ?>%"<?php endif; ?> >
            ...
            <!-- 列表 -->
              ...
              <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><li>
                  <div class="li_top" colspan="6"><?php if($check_all): ?><input class="ids regular-checkbox" type="checkbox" value="<?php echo ($data['id']); ?>" name="ids[]" id="check_<?php echo ($data['id']); ?>"><label for="check_<?php echo ($data['id']); ?>"></label><?php endif; ?>订单号:<?php echo ($data["order_number"]); ?> &nbsp;&nbsp;&nbsp;&nbsp;付款时间:<?php echo (time_format($data["pay_time"])); ?>
                  &nbsp;&nbsp;<span class="colorless">|</span>&nbsp;&nbsp;&nbsp;<span class="li_top_u"><img width="30" src="<?php echo ($data["headimgurl"]); ?>"/><?php echo ($data["uid"]); ?></span>
                  </div>
                  <div class="li_main">
                      <div class="goods"><?php echo ($data["goods"]); ?></div>
<!--                       <div class="price">￥<?php echo ($data["market_price"]); ?></div> -->
                     
                      <div class="total_price">￥<?php echo ($data["total_price"]); ?>
                     
                      	<?php if($data['order_from_type'] != '商城'): ?><br/><span class="red_text_bg"><?php echo ($data["order_from_type"]); ?></span><?php endif; ?>
                      </div>
                       <div class="num"><?php echo ($data["num"]); ?></div>
<!--                         <div class="status_name"> -->
<!--                         	<?php if(($data["is_deposit"]) == "0"): ?>-->
<!--                         		全额支付 -->
<!--                         		<?php else: ?> -->
<!--                         		定金支付 -->
<!--<?php endif; ?> -->
<!--                         </div> -->
                      <div class="status_name">
                      	<?php echo ($data["status_code_name"]); ?>
                      	<span style="color: #A7A4A7; display: block; padding: 5px;">
                      		<?php if($data['order_state'] == 1): ?>（正常）
                      ...
                      <div class="status"><?php echo ($data["action"]); ?>
                      <br/><br/>	<a href="javascript:;" onclick="do_del(<?php echo ($data["id"]); ?>)">删除</a>
                      </div>
                 	</div> 
...
(function(){
  ...
  function do_del(id){
	  if(confirm('确定要删除该订单么？')){
		  window.location.href="<?php echo addons_url('Shop://Order/del',array('mdm'=>$_GET['mdm']));?>&id="+id;
	  }
  }
$(function(){
	//搜索功能
	...
	$("select[name='users']").change(function(){
		var url = '<?php echo addons_url("Shop://Order/lists");?>&mdm=<?php echo ($_GET["mdm"]); ?>&users='+$(this).val();
