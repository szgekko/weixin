      ...
      <?php if($add_button || $del_button || $search_button || !empty($top_more_button)): ?><div class="table-bar">
        ...
        <div style="float:left;margin-right:10px;">
        	<select style="width: 120px;" id='search_by_level'>
        		...
        		<?php if(is_array($typeName)): $i = 0; $__LIST__ = $typeName;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tt): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if($_GET['level_key'] == $key): ?>selected=selected<?php endif; ?> ><?php echo ($tt); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        	</select>
        </div>
        <div style="float: left; margin-right: 20px;">
     &nbsp;&nbsp;  创建日期 <input type="datetime" placeholder="请选择创建日期" value="<?php echo ($_GET['start_ctime']); ?>" class="text date" name="start_ctime" style="width: 150px;">
            至
            <input type="datetime" placeholder="请选择创建日期" value="<?php echo ($_GET['end_ctime']); ?>" class="text date" name="end_ctime" style="width: 150px;">
...
<!--<?php endforeach; endif; else: echo "" ;endif; ?> -->
					...
					<td>
					<?php if(($data["is_audit"]) != "1"): ?><a onClick="$.WeiPHP.openSubmitDialog('分销商用户审核','<?php echo U('set_user_level?model='.$model.'&id='.$data[id]);?>',600,400)" href="javascript:;">审核</a>&nbsp;&nbsp;
					<?php else: ?>
					<a href="<?php echo U('user_detail',array('did'=>$data['id'],'mdm'=>$_GET['mdm']));?>">详情</a>&nbsp;&nbsp;<?php endif; ?>
					
					<a href="<?php echo U('edit',array('id'=>$data['id'],'model'=>$model,'mdm'=>$_GET['mdm']));?>">编辑</a>&nbsp;&nbsp;
					<a href="javascript:;" onClick="do_del_user('<?php echo ($data["id"]); ?>','<?php echo ($data["level"]); ?>','<?php echo ($data["truename"]); ?>')">删除</a>&nbsp;&nbsp;
					<a class="shop_qrcode" href="<?php echo ($data["qr_code"]); ?>">分销商二维码</a>
					</td>
...
$(function(){
	...
	$("select[name='pay_type']").change(function(){
		var url = '<?php echo addons_url("Payment://PaymentOrder/lists");?>&mdm=<?php echo ($_GET["mdm"]); ?>&pay_type='+$(this).val();
	...
	$("select[name='pay_status']").change(function(){
		var url = '<?php echo addons_url("Payment://PaymentOrder/lists");?>&mdm=<?php echo ($_GET["mdm"]); ?>&pay_status='+$(this).val();
	...
	$('#priceSearch').click(function(){
		...
		var url="<?php echo addons_url('Payment://PaymentOrder/lists');?>&is_price=1&min_value="+min+"&max_value="+max;
		location.href=url;
	});
    //回车自动提交
    ...
    $('#ctimeSearch').click(function(){
		...
		if(start || end){
			var url="<?php echo addons_url('Shop://ShopDistributionUser/lists');?>&is_ctime=1&start_ctime="+start+"&end_ctime="+end;
			location.href=url;
		}
    ...
    $("#search_by_level").change(function(){
    	...
    	var url="<?php echo addons_url('Shop://ShopDistributionUser/lists');?>&level_key="+levelkey;
		location.href=url;
    });
...
function do_del_user(did,level,truename){
	if(confirm("用户 ‘"+truename+"’ 是 ‘"+level+"’ 级别分销商，若删除该用户其以下级别用户也将删除，删除后不可恢复数据，请慎重操作！        确定即删除数据")){
		$.post("<?php echo U('do_del_duser');?>",{'id':did},function(res){
			if(res==0){
				alert('数据找不到！');
			}else if(res==1){
				alert('删除成功！');
