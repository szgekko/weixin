...
class WeixinAddonModel extends WeixinModel {
	function reply($dataArr, $keywordArr = array()) {
		$config = getAddonConfig ( 'ShopCoupon' ); // 获取后台插件的配置参数
			                                           // dump($config);
			                                           
		$map ['token'] = get_token ();
		$keywordArr ['aim_id'] && $map ['id'] = $keywordArr ['aim_id'];
		$data = M ( 'shop_coupon' )->where ( $map )->find ();
		
		// 其中token和openid这两个参数一定要传，否则程序不知道是哪个微信用户进入了系统
		$param ['token'] = get_token ();
		$param ['openid'] = get_openid ();
		$param ['id'] = $data ['id'];
		$url = addons_url ( 'ShopCoupon://Wap/prev', $param );
		...
		if (empty ( $data ['end_time'] ) || $data ['end_time'] > $now) {
		    $articles [0] ['Description'] = $data ['content'];
	...
	function personal() {
		$links = array (
				'url' => addons_url ( 'ShopCoupon://Wap/personal' ), // 链接地址
				'title' => '商城代金券', // 链接名称
				'icon' => '', // 图标，选填
