...
class WapController extends AddonsController {
	...
	function personal() {
		$can_use = I ( 'get.use' ,1,'intval');
// 		if ($isUse != '') {
// 			$can_use = $isUse;
// 		}
		
		$list = D ( 'Common/SnCode' )->getMyAll ( $this->mid, 'Coupon', true, '', $can_use );
		$cDao = D ( 'Addons://Coupon/Coupon' );
		if (! empty ( $list )) {
			foreach ( $list as $k => &$v ) {
				$coupon = $cDao ->getInfo ( $v ['target_id'] );
				//&& $coupon['over_time']>NOW_TIME
				if ($coupon ) {
					if ($coupon['over_time'] <= NOW_TIME){
					    $v['can_use']=0;
