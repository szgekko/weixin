...
class WeixinController extends AddonsController {
	...
	function getPaymentOpenid() { // echo '444';
		...
		if (! isset ( $_GET ['getOpenId'] )) {
			...
			addWeixinLog ( $content, 'checkpayopenid_' . $param ['appid'] . '_' . $param ['secret'] );
	...
	public function pay() {
		...
		addWeixinLog ( $this->payConfig, 'paytesturl5_' . $token );
		addWeixinLog ( $order, 'paytesturl6_' . $token.'_'.$orderNo );
	...
	public function payOK() {
		...
		// dump($paymentId);
		// dump($isPay);
		// addWeixinLog($isPay,'dopayteststatue_'.$this->mid);
		if ($isPay) {
