...
class LuckyFollowController extends AddonsController {
	function _initialize() {
		...
		$res ['title'] = '抽奖游戏活动';
		$res ['url'] = addons_url ( 'Draw://Games/lists',array('mdm'=>$_GET['mdm']));
		$res ['class'] = _CONTROLLER == 'Games' ? 'current' : '';
		$nav [] = $res;
		
		$res ['title'] = '奖品库管理';
		$res ['url'] = addons_url ( 'Draw://Award/lists',array('mdm'=>$_GET['mdm']));
		$res ['class'] = _CONTROLLER == 'Award' ? 'current' : '';
		$nav [] = $res;
		
		$res ['title'] = '中奖人列表';
		$res ['url'] = addons_url ( 'Draw://LuckyFollow/games_lucky_lists' ,array('mdm'=>$_GET['mdm']));
...
// 	        $v['state']=$v['state']==1?'已发奖':'未发奖';
	    ...
	    if (empty($gamesId)){
	        ...
	        $amap['aim_table']='lottery_games';
	        $awardDA = D('Addons://Draw/Award')->where($amap)->select();
	    }else {
	         $awards=$awardDao->getGamesAwardlists($gamesId);
	         foreach ($awards as $aa){
	             $aw['id']=$aa['award_id'];
	             $aw['name']=$aa['name'];
	             $awardDA[]=$aw;
	         }
	    ...
	    $this->assign('award_data',$awardDA);
	    $this->display ();
	}
