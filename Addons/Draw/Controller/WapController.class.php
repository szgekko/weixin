...
class WapController extends AddonsController {
	...
	function index() {
	    ...
	    //所有获奖列表
	    $luckLists=D('Addons://Draw/LuckyFollow')->getGamesLuckyLists($gameId,0,'-1','lottery_games',5);
	    //个人获奖列表
	    $uid=$this->mid;
		if ($uid > 0) {
	    ...
	    switch ($info['game_type']){
	        ...
	        case 4:
	            ...
	            if ($count >= 10){
	                ...
	                $nicejpNum = 0;
	                foreach ($awardLists as $jpl1){
	                    if ($nicejpNum < 9){
	                        $awardLists1[]=$jpl1;
	                    }
	                    $nicejpNum++;
	                }
	                $awardLists = $awardLists1;
	                $jplist = $jdata;
	                $count=count($jplist);
	            }
	            $num=10-$count;
...
// 	    addWeixinLog($map,'drawtest1111111');
// 	    addWeixinLog($has_subscribe,'drawtest1111111');
