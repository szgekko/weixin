...
// 	            }
	...
	//同步ERP会员信息 
	function updateERPMember($cardInfo,$uid=0,$openid='',$levelInfo=NULL){
		$msg='';
	    //对接erp
	    $managerId=session ( 'manager_id');
	    $userInfo=get_userinfo($managerId);
	    if ($userInfo['secretID'] && $userInfo['secretKey']){
	        ...
	        $severDao = D ( 'Common/Server' );
	        $member=$severDao->checkMemberByMobile ($cardInfo['phone']);
			$member2=$severDao->checkMemberByOpenID ($openid);
			if ($member['Rows'][0]['状态']==0 && $member2['Rows'][0]['状态']==0){
	        	return $this->addERPMember($cardInfo,$uid,$openid);
	        	
	        }else if(($member['Rows'][0]['状态']==1 && $member2['Rows'][0]['状态'] == 0) || $member2['Rows'][0]['状态']!=0){
...
// 	        		return $resdata;
// 	        	}
	        	$oldMobile=$member2['Rows'][0]['电话号码'];
	        	if ( $member2['Rows'][0]['状态']!=0 && $oldMobile != $cardInfo['phone'] && $member['Rows'][0]['状态'] >0 ){
	        		$resdata['res']=0;
	        		$resdata['msg']='保存失败！该手机号已是其他用户的手机号！';
	        		return $resdata;
	        	}
	        	// 通过手机号同步OPENID
				$bindstatus=$severDao->updateOpenIDByMobile($cardInfo['phone'], $openid);
				if(strstr($bindstatus['Rows'][0]['状态'],'绑定成功')){
					...
					$param ['CustType'] = $levelInfo['level']; // 1
					...
					addWeixinLog($param,'erptongbu1111');
					$res1=$severDao->updateMemberInfoByOpenID ($openid, $param);
					//dump($res1);
					//exit;
...
// 						$user_score=get_userinfo($uid,'score');
	...
	function addERPMember($cardInfo,$uid=0,$openid='',$levelInfo=NULL){
		$msg='';
	    //对接erp
	    $managerId=session ( 'manager_id');
	    $userInfo=get_userinfo($managerId);
	    if ($userInfo['secretID'] && $userInfo['secretKey']){
	        ...
	        if ($member['Rows'][0]['状态']==0 && $member2['Rows'][0]['状态']==0){
	            ...
	            $param ['Birthday'] = time_format($cardInfo['birthday'],'Y-m-d'); // 2015-10-15
	            $param ['MarryDay'] =''; // 2015-09-15
	            $res1=$severDao->InsertMemberInfo ($openid, $param);
	            if(strstr($res1['Rows'][0]['状态'],'保存失败')){
	                //保存失败
	            	$msg=$res1['Rows'][0]['状态'];
	            	$res=0;
