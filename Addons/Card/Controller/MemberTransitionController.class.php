...
class MemberTransitionController extends BaseController {
	...
	function recharge_lists() {
		...
		foreach ( $list_data ['list_data'] as &$vo ) {
			...
			$vo ['member_id'] = empty($cardMember ['number']) ? $vo['number'] : $cardMember ['number'];
			$vo ['username'] = empty($cardMember ['username']) ? $vo['username'] : $cardMember ['username'];
			
			$vo ['phone'] = empty($cardMember ['phone']) ? $vo['phone'] : $cardMember ['phone'] ;
			$vo ['branch_id'] = $vo ['branch_id'] == 0 ? '商店总部' : $branch [$vo ['branch_id']];
		}
		
		// dump($uInfo);
		unset($list_data['list_grids']['number']);
	...
	function buy_lists() {
		...
		if (! empty ( $list_data ['list_data'] )) {
			...
			foreach ( $list_data ['list_data'] as &$vo ) {
				...
				$vo ['member_id'] =  empty($cardMember ['username']) ? $vo['username'] : $cardMember ['username'];
		...
		unset($list_data['list_grids']['username']);
...
// 			$nickname_follow_ids = implode ( ',', $nickname_follow_ids );
		...
		$data['_page'] && $list_data ['_page'] = $data['_page'];
		// dump($list_data['list_data']);
		...
		foreach ( $list_data ['list_data'] as &$vo ) {
			...
			}else if($vo['credit_title']){
			    $vo ['credit_name'] = $vo['credit_title'];
			    $vo ['operator'] = '用户自己';
			}else {
			    if ($vo ['credit_name'] == 'signin'){
			        $vo ['credit_name'] = '签到';
			    }else if($vo ['credit_name'] == 'shoppay'){
			        $vo ['credit_name'] = '到店支付返积分';
			    }else if($vo ['credit_name'] == 'Reserve'){
			        $vo ['credit_name'] = '微预约';
			    }else if($vo ['credit_name'] == 'shop_reward'){
			        $vo ['credit_name'] = '促销活动';
			    }else {
	...
	function score_tongji() {
		...
		foreach ($get_data as $gg){
		    $getD[$gg['allday']]=round ( floatval ($gg['totalScore']), 2);
		}
		foreach ($use_data as $uu){
		    $useD[$uu['allday']]= 0-round ( floatval ($uu['totalScore']), 2);
		}
...
// 			// $getAllDay[]=$v['allday'];
// 			if ($dateArr [$v ['allday']]) {
// 				$val = round ( floatval ( $v ['totalScore'] ), 2 );
// 			}
// 			$getAllScore[]=$val;
// 		}
...
// 			// $useAllDay[]=$v['allday'];
// 			if ($dateArr [$v ['allday']]) {
// 				$val = 0 - round ( floatval ( $v ['totalScore'] ), 2 );
// 			}
