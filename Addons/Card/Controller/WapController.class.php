...
// 		    }
	...
	function me(){
		...
		if ($info) {
			...
			$this->assign('orderCount',count($myorders));
			$this->assign ( 'info', $info );
	...
	function getLot()
    {
        ...
        if (empty($lotArr)){
            ...
            if ($info['status'] == "0") {
                $lotArr['latitude'] = $info['content']['point']['y'];// 纬度
                $lotArr['longitude'] = $info['content']['point']['x'];// 经度
                S($key,$lotArr,86400);
            }
        }           
        return $lotArr;
...
// 	        $longitude=I('longitude');
	        $latitude= $lotArr['latitude'];
	        $longitude=$lotArr['longitude'];
	        addWeixinLog($latitude,$longitude);
	        $map['token']=get_token();
...
// 		}
		...
		$token = get_token();
		$cardMap ['uid'] = $this->mid;
		$cardMap['token']=$token;
		$cardMap['status'] = 1;
		$info = M ( 'card_member' )->where ( $cardMap )->find ();
		if ($info){
		    $levelInfo = D('CardLevel')->getCardMemberLevel($this->mid);
		}
...
// 		$map['member'] = array('in',array(0,-1,$levelInfo['id']));
// 		$row = empty ( $model ['list_row'] ) ? 20 : $model ['list_row'];
		...
		foreach ($data as $key=>$ddd){
		    !empty($user['sex']) && $sex= $user['sex'] == 1?-3:-2;
		    $member = explode(',', $ddd['member']);
		    if (in_array(0, $member) || in_array(-1, $member) && $info || $sex && in_array($sex, $member)){
